...
var Conv = require('./models/conversation');
...
//Logic part
   io.on('connection', function(socket){
     socket.on('teaching', function(data){
       ...
       console.log(ans);
       var newSentence = Conv({ sentence: que, tags: arr,  answer: ans });
       newSentence.save(function(err) {if (err) throw err;});
     });
     ...
     socket.on('chating', function(datai){
       ...
       Conv.find({tags: {$all: arr}}, function(err, docs) {
         if (err) throw err;
         if (typeof docs[0] !== 'undefined' && docs[0] !== null) {
           ...
           var newSentence = Conv({ sentence: chatq, tags: arr,  answer: "I don't know!" });
           newSentence.save(function(err) {if (err) throw err;});
         }
       });
     });
